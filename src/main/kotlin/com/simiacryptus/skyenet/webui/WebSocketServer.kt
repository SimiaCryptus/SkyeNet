...
abstract class WebSocketServer(private val resourceBase: String) {
    ...
    abstract class SessionStateByID(sessionId:String) : SessionState(sessionId) {
        private val sentMessages = LinkedHashMap<String, String>()

        override fun send(out: String) {
            try {
                logger.debug("$sessionId - $out")
                val split = out.split(',', ignoreCase = false, limit = 2)
                sentMessages.put(split[0], split[1])
                publish(out)
            } catch (e: Exception) {
                logger.error("$sessionId - $out", e)
            }
        }
    ...
    inner class MessageWebSocket(
        ...
        override fun onWebSocketClose(statusCode: Int, reason: String?) {
            ...
            logger.info("$sessionId - Socket closed: [$statusCode] $reason")
