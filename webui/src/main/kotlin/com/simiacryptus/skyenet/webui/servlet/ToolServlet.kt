  ...
  private fun toolDetailsPage(tool: Tool) = """
          ...
          <body>
            ...
            <div class='tool-details'>
              <div>Path: <div class='tool-path'>${tool.path}</div></div>
              <div>Interpreter: <div class='interpreter-display'>${tool.interpreterString}</div></div>
              <div>API Description: <div class='json-display'>${JsonUtil.toJson(tool.openApiDescription)}</div></div>
              <div>Code: <div class='code-display'>${MarkdownUtil.renderMarkdown("```kotlin\n${tool.servletCode}\n```")}</div></div>
            </div>
          </body>
          </html>
        """.trimIndent()
  ...
  private val header
    get() = """<meta charset="UTF-8">
              ...
              <style>
                ...
                h1 { color: #5a5a5a; }
                .tools, .tool-details { max-width: 960px; margin: auto; }
                .tool, .edit-form { background-color: #fff; border: 1px solid #e1e1e1; border-radius: 4px; padding: 20px; margin-bottom: 20px; }
                .tool-path, .edit-form input[type="text"], .edit-form textarea { font-weight: 700; color: #2a2a2a; }
                .json-display, .code-display, .edit-form textarea { background-color: #fafafa; border: 1px solid #e1e1e1; border-radius: 4px; padding: 10px; white-space: pre-wrap; overflow: auto; }
                .edit-form label { display: block; margin-top: 20px; }
                .edit-form input[type="text"], .edit-form textarea { width: 100%; box-sizing: border-box; }
                .edit-form input[type="submit"] { margin-top: 20px; background-color: #4CAF50; color: white; padding: 10px 24px; border: none; border-radius: 4px; cursor: pointer; }
